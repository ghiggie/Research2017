program rw
   ...
   ! The following if statement checks to see whether you supply any
   ! flags to the compiler. The possible flags are "debug" and 
   ! "newseed". The debug flag will output information about the
   ! running of the program and will introduce additional stop
   ! commands. The newseed flag ensures that all the random numbers are
   ! generated from a different seed. This is something that we would
   ! like to turn off during testing. 
   ...
   ! open a file for debug information, iff the debug flag is issued
   if(debug) then
   ...
   if(debug) call cell_test(n_walkers)  ! check the consistency in cell assignment
